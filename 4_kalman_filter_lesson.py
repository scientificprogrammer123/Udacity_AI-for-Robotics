...
# function over the space of locations, the area underneath sums to 1. It is
# characterized by mean (mu) and variance (sigma^2). This is a unimodal.

# Equation is: (2*pi*sigma^2)^(-0.5) * exp(-0.5*(x-mu)^2/sigma^2)
# they called sigma^2 the covariance, why not variance?
...
#13 prior has mean (mu) and variance (sigma^2)
# measurement has mean(v) and covariance (r^2)
# The new mean and variance are calculated as:
# new mean (mu') =  (r^2*mu + sigma^2*v) / (r^2 + v^2)
# new variance (sigma^2') = (r^(-2) + v^(-2))^(-1)
...
# measurement update: baye's rule + multiplication, this is in #17
# motion update prediction: total probability + addition
# mu' = mu + u = 8 + 10 = 18
# sigma^2' = sigma^2 + r^2 = 4 + 6 = 10
...
def update(mean1, var1, mean2, var2):
    ...
    new_var = 1/(1/var1 + 1/var2)
    return [new_mean, new_var]
# just add them together, this is the 1D kalman filter
...
#22 multivariate gaussians, kalman filter land
# 2 dimensions, position and velocity
...
# Because those 2 things interact, subsequent observations of the observable 
# variables, gives us information about the hidden variables, so we can estimate
# the velocities as well.

#26 kalman filter design
# When you design a kalman filter you need effectively 2 things. For the state, you 
...
# We know the new location is the old location + velocity, matrix is [1 1; 0 1],
# where the state vector is [x_dot, x]. 
# For the measurement, we only observe the first component of the place, not velocity, 
# so the matrix is v = [1 0]*[x_dot x]. matrix F and H.
#
...
#27 kalman matrices (NOT COMPLETE, moving onto particle filters, COME BACK TO THIS, and also the assignment)
